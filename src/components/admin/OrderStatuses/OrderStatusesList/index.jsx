...
import AddNewOrderStatusModal from './AddNewOrderStatusModal';
import { DeleteOrderStatus } from '@/services/orderstatuses';

function OrderStatusesList(props) {
  const [orderstatuses, setOrderStatuses] = useState(props.orderstatuses);
  const [addNewOrderStatusModal, setAddNewOrderStatusModal] = useState(false);
  const [, setEditOrderStatusModal] = useState(false);
  const [editOrderStatus, setEditOrderStatus] = useState(false);

  const handleOrderStatusEdit = async (orderstatus) => {
    setEditOrderStatus(orderstatus);

    orderstatus && setEditOrderStatusModal(true);
    setEditOrderStatusModal(true);
  };

  const handleDeleteOrderStatus = async (orderstatusId) => {
    const response = await DeleteOrderStatus(orderstatusId);

    console.log('RESPONSE123: ', response);

    if (response.status === 200) {
      setOrderStatuses((prevOrderStatuses) =>
        prevOrderStatuses.filter(
          (orderstatus) => orderstatus.ID !== orderstatusId
        )
      );
    }
  };
  ...
  return (
    <div className='flex-1 bg-white text-gray-600 rounded overflow-hidden shadow-lg border'>
      {addNewOrderStatusModal ? (
        <AddNewOrderStatusModal closeModal={setAddNewOrderStatusModal} />
      ) : null}

      {editOrderStatus ? (
        <EditOrderStatusModal
          editOrderStatus={editOrderStatus}
          closeModal={setEditOrderStatusModal}
        />
      ) : null}
        ...
        <div className='relative overflow-x-auto shadow-md sm:rounded-lg py-2 px-4'>
          <div className='flex items-center py-4 bg-white '>
            ...
            <button
              ...
              onClick={() => {
                setAddNewOrderStatusModal(true);
              }}
            >
              Yeni
            </button>
                ...
                orderstatuses.map((orderstatus) => {
                  return (
                    <OrderStatusListItem
                      ...
                      handleDeleteOrderStatus={handleDeleteOrderStatus}
                      handleOrderStatusEdit={handleOrderStatusEdit}
                    />
                  );
                })}
            </tbody>
