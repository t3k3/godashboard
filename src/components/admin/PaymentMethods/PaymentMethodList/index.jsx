...
import EditEftPaymentMethodModal from './EditEftPaymentMethodModal';
import EditPaytrPaymentMethodModal from './EditPaytrPaymentMethodModal';
import EditKapidaPaymentMethodModal from './EditKapidaPaymentMethodModal';
...
function PaymentMethodList(props) {
  ...
  const [editEftPaymentMethodModal, setEditEftPaymentMethodModal] =
    useState(false);
  const [editKapidaPaymentMethodModal, setEditKapidaPaymentMethodModal] =
    useState(false);
  const [editPaytrPaymentMethodModal, setEditPaytrPaymentMethodModal] =
    useState(false);
  const [paymentMethodData, setPaymentMethodData] = useState(false);
  ...
  const handlePaymentMethodEdit = (paymentMethod) => {
    ...
    if (paymentMethod.edit == 'bank_transfer') {
      setEditEftPaymentMethodModal(true);
    } else if (paymentMethod.edit == 'cod') {
      setEditKapidaPaymentMethodModal(true);
    } else if (paymentMethod.edit == 'paytr_checkout') {
      setEditPaytrPaymentMethodModal(true);
    } else {
      return;
    }
  };
  ...
  return (
    <div className='flex-1 bg-white text-gray-600 rounded overflow-hidden shadow-lg border'>
      ...
      {/* PAYTR EDİT PAYMENT MODAL */}
      {editPaytrPaymentMethodModal ? (
        <EditPaytrPaymentMethodModal
          // product={product}
          // setProduct={setProduct}
          paymentMethodData={paymentMethodData}
          setPaymentMethodData={setPaymentMethodData}
          closeModal={setPaymentMethodData}
        />
      ) : null}

      {/* HAVALE/EFT EDİT PAYMENT MODAL */}
      {editEftPaymentMethodModal ? (
        <EditEftPaymentMethodModal
          // product={product}
          // setProduct={setProduct}
          paymentMethodData={paymentMethodData}
      ...
      {/* KAPIDA ODEME EDİT PAYMENT MODAL */}
      {editKapidaPaymentMethodModal ? (
        <EditKapidaPaymentMethodModal
          // product={product}
          // setProduct={setProduct}
          paymentMethodData={paymentMethodData}
      ...
      <div className='mx-4 my-2'>
        <div className=' flex justify-between h-6  text-gray-600 font-medium'>
          <span className='px-2 text-sm font-semibold'>Ödeme Yöntemleri</span>
        </div>
                ...
                paymentMethods.map((paymentMethod) => {
                  return (
                    <PaymentMethodListItem
                      ...
                      handlePaymentMethodEdit={handlePaymentMethodEdit}
                    />
                  );
                })}
            </tbody>
